...
class Notifications:
		...
		def msg_improper_input(self):
				print "ERROR:\t\t Why you annoying me with improper inputs? Go, Try again!\n"

		# Enter starting coordinates prompt
		def msg_enter_origin_coordinates(self):
				print "\nEnter the coordinates of the dice you want to move (E.g. 1 1) :- "

		# Enter destination square prompt
		def msg_enter_destination_coordinates(self):
				print "Enter the coordinates of the destination (E.g. 5 4) :- "

		# Input out of bounds error msg
		def msg_input_out_of_bounds(self):
				print "ERROR:\t\t Input co-ordinates out of bound. *Rolls eyes* Try again!\n"
		
		# Moving another player's dice error msg
		def msg_wrong_dice(self):
				print "ERROR:\t\t Woah! Foul! That ain't your dice to move homie!\n"
		
		# No dice to move error msg
		def msg_no_dice_to_move(self):
				print "ERROR:\t\t Don't you see there is no dice to move in that co-ordinate?\n"
		
		# Invalid move error msg
		def msg_invalid_move(self):
				print "ERROR:\t\t Why you always trying Invalid Moves?\n"
		
		# Trying to run own dice error msg
		def msg_running_over_own_dice(self):
				print "MSG:\t\t Are you really trying to capture your own dice, bonehead?\n"
		...
		def msg_no_valid_path(self):
				print "ERROR:\t\t Yo numskull! NO Valid Path was found to get to your selected destination. BOOO!\n"
		
		# Enter path choice for 90 degree turns prompt
		def msg_90degree_path_selection(self):
				print "90 Degree turn detected. Enter 1 to go vertically first, or 2 to go laterally first :- "
		
		# 90 degree turns re-routed msg
		def msg_90degree_path_selection_not_processed(self):
				print "MSG:\t\t We're sorry, but your DUMB path selection for the 90 Degree turn was Invalid.\n"
		...
		def msg_nature_of_path_taken(self, path):
				print "MSG:\t\t A %s path was taken to get to the destination\n" %(path)	

		# Displaying start and end coordinates of the move
		"""def Msg_MoveDescription(int startRow, int startCol, int endRow, int endCol, int topValueAtStart, int rightValueAtStart, int topValueAtEnd, int rightValueAtEnd, bool isBotOperated):
			char player
			...
			print "ACTION:\t\t The dice "  player  topValueAtStart  rightValueAtStart  " in ("  startRow  ", "  startCol  ") was moved to ("  endRow  ", "  endCol  "). It is now "  player  topValueAtEnd  rightValueAtEnd  "\n"
			print "\t\t There were "  abs(startRow - endRow)  " vertical rolls & "  abs(startCol - endCol)  " horizontal rolls made."  "\n"
			"""

		# Crash msg
		def msg_crashed_while_making_the_Move(self):
				print "ERROR:\t\t Whoopsie Daisy! The program crashed while making the move.\n"
		
		# Displays toss results
		def msg_toss_results(self, winner):
		...
		def msg_serialize_prompt(self):
				print "Want to Serialize? Press 'y' to serialize, 'n' to continue :- "
		...
		def msg_serialized(self, status):
				print "Serialization %s. The game will exit now.\n" %(status)
				self.draw_divider()
		...
		def botsthink_hostile_opponent_uncapturable(self, whosUnderThreat):
				print "Bots Mumbling:\t That hostile opponent aiming to attack couldn't be captured. Trying alternatives...\n" %(whosUnderThreat)	

		# Blocking move successful msg
		def botsthink_BlockingMoveMade(self):
